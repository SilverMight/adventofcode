...
func main() {
	...
	for seedIndex := 0; seedIndex < len(seedsArray)-1; seedIndex += 2 {
		lengthIndex := seedIndex + 1

		seed64, _ := strconv.ParseInt(seedsArray[seedIndex], 10, 64)
		length, _ := strconv.ParseInt(seedsArray[lengthIndex], 10, 64)
		//fmt.Println(seed64, length)
		for curSeed := seed64; curSeed < seed64+length; curSeed++ {
			//fmt.Println("Now checking ", curSeed)
			curLoc := getLocation(curSeed, sourceDestMap)
